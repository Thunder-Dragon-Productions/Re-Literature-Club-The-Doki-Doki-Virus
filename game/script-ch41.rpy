...
label ch41_start:
    ...
    s "..."
    s "..."
    mc "You're kind of acting weird all of a sudden, Sayori."
    show sayori at h21
    s "!!!"
    s "..."
    mc "What made you think that--{nw}"
    mc "Wait? What's going--"
    mc "Ouch! Holy crud, my head!"
    mc "What are these images flowing in my head all of a sudden?"
    mc "Ah!"
    $ quick_menu = False
    $ style.say_window = style.window_occult
    $ style.namebox = style.namebox_occult
    $ kai_name = "??? 1"
    $ ks_name = "??? 2"
    oc_mc2 "Where am I?"
    kai "Here."
    ...
    mc "What are these images flowing in my head all of a sudden?"
    mc "Ah!"
    ## The text windows during the Occult Club visions
    ## The following scene below takes place in the Occult Club's gaming dimension during their Doki Doki -Occult- Torture Club! video game. As such, Club Presdients and Club Founders of other clubs (excluding the Gaming Club) only see these scripts to be invisible and undetectable as they are not part of their club's database. For more details, see "scriptsuppressor.init".
    # $ quick_menu = False
    # $ style.say_window = style.window_occult
    # $ style.namebox = style.namebox_occult
    # [content]
    ...
    # $ style.say_window = style.window
    # $ style.namebox = style.namebox0
    ## This is the end of the vision to the Occult Club. While the above scene cannot be read by non-Occult Club members (except for the Gaming Club President if available), the Literature Club can read everything else in this script. These rules are applied to all Occult Club visions.
    return
